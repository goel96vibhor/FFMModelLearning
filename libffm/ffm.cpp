...
ffm_model init_model(ffm_int n, ffm_int m, ffm_parameter param)
{
    ...
    ffm_int align0 = 2 * get_k_aligned(model.k);
        ffm_int align1 = model.m * align0;
    w = model.W;
    int feature_name_ids[16] = {0,1,2, 130236, 84850, 84940, 85252, 85253, 88939, 88946, 88947, 88954, 88962, 95376, 95382, 95383};
        for (int i=0;i<16;i++)
		    {
			            cout<<"id:"<<feature_name_ids[i]<<endl;
				    cout<<std::fixed;
				            ffm_float *w1 = model.W + (ffm_long)feature_name_ids[i]*align1;
					            for(ffm_int d = 0; d < align1/2.0; d += kALIGN)
							                cout <<std::setprecision(4)<<  w1[d]<<" ";
					            cout<<endl;
						        }
    return model;
}
...
void txt2bin(string txt_path, string bin_path) {
    ...
    while(fgets(line.data(), kMaxLineSize, f_txt)) {
        ...
        for(; ; p++) {
            ...
            scale += N.v*N.v;
	    //cout<<std::fixed;
	    //cout<<N.f<<" "<<N.j<<" "<<std::setprecision(8)<<N.v<<endl;
        }
        scale = 1.0 / scale;
	//cout<<y<<" "<<scale<<endl;
...
void ffm_save_model(ffm_model &model, string path) {
    ...
    cout<<"n:"<<model.n<<" ,m:"<<model.m<<" ,k:"<<model.k<<endl;
    ffm_long w_size = get_w_size(model);
